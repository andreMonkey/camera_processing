...
def deleteCallback(n): # Delete confirmation
  ...
  screenModePrior = -1
  if n is True:
    os.remove(pathData[storeMode] + '/IMG_' + '%04d' % loadIdx + '.png')
...
def imgRange(path):
  ...
  try:
    for file in os.listdir(path):
      if fnmatch.fnmatch(file, 'IMG_[0-9][0-9][0-9][0-9].png'):
        i = int(file[4:8])
        if(i < min): min = i
        if(i > max): max = i
  finally:
...
def takePicture():
  ...
  while True:
    filename = pathData[storeMode] + '/test_' + '%04d' % saveIdx + '.png' # trying to save in png so that pixelsorting goes automatically
...
def showNextImage(direction):
  ...
  while True:
    ...
    if os.path.exists(pathData[storeMode]+'/IMG_'+'%04d'%n+'.png'):
      showImage(n)
      break
...
def showImage(n):
  ...
  img      = pygame.image.load(
              pathData[storeMode] + '/IMG_' + '%04d' % n + '.png')
  scaled   = pygame.transform.scale(img, sizeData[sizeMode][1])
  loadIdx  = n
...
#TODO save image as rgba/png so it doesnt need to be converted by pixelsort.py
